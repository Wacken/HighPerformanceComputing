...
__global__ void matrixMultKernel_overlap(float* Ad, float* Bd, float* Cd, int n)
{
	__shared__ float Ads[TILE_SIZE][TILE_SIZE];
	__shared__ float Bds[TILE_SIZE][TILE_SIZE];
	float Atmp[TILE_SIZE][TILE_SIZE];
	float Btmp[TILE_SIZE][TILE_SIZE];
	...
	Atmp[ty][tx] = Ad[i*n + tx];
	Btmp[ty][tx] = Bd[ty*n + k];
	__syncthreads();

	float Celem = 0;

	for (int m = 0; m < n / TILE_SIZE - 1; m++) 
	{
		
		Ads[ty][tx] = Atmp[ty][tx];
		Bds[ty][tx] = Btmp[ty][tx];
		__syncthreads();

		Atmp[ty][tx] = Ad[i*n + m * TILE_SIZE + tx];
		Btmp[ty][tx] = Bd[(m*TILE_SIZE + ty)*n + k];
